...
if(!file_exists("$catalog/admin/db/$_GET[id]/index.txt")) exit();



$conf=file("$catalog/admin/conf/mes.txt");
...
$content=file("$catalog/admin/db/$_GET[id]/index.txt");
...
$file=array();
$d=opendir("$catalog/admin/db/$_GET[id]");
...
$top_code="";
$bot_code="";
if(filesize("$catalog/admin/conf/code/top.txt")!=0)
  {
    $f=fopen("$catalog/admin/conf/code/top.txt","r");
    $top_code=fread($f,filesize("$catalog/admin/conf/code/top.txt"));
    fclose($f);
  }

if(filesize("$catalog/admin/conf/code/bot.txt")!=0)
  {
    $f=fopen("$catalog/admin/conf/code/bot.txt","r");
    $bot_code=fread($f,filesize("$catalog/admin/conf/code/bot.txt"));
    fclose($f);
  }
...
<head>
  <title>Комментарии <?echo $content[0]?></title>
  <style>
  .navig_activ
         {
...
<tr><td>
<? if($conf[6]==1) echo "<div id=head>$content[0]</div>"; ?>
 ...
 for($x=$start,$y=0; $x<count($file); $x++,$y++)
         {
           ...
           $comm=file("$catalog/admin/db/$_GET[id]/$file[$x]");
